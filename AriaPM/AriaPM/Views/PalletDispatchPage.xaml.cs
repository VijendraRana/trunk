...
namespace AriaPM.Views
{
    ...
    public partial class PalletDispatchPage : ContentPage
    {
        ...
        public PalletDispatchPage()
        {
            ...
            viewModel.Title = "Pallet Dispatch";
        }
        ...
        protected override void OnAppearing()
        {
            ...
            {
                viewModel.LoadItemsCommand.Execute("Booked");
            }
        }
        ...
        private async void btnDispatchPallet_Clicked(object sender, EventArgs e)
        {
            if (ItemIds.Count > 0 && SelectedShipper?.SelectedItem != null && !string.IsNullOrWhiteSpace(ConsigmentNumber.Text))
            {
                var result = viewModel.UpdatePalletStatusWithShipper(ItemIds, "dispatched", ((PickList)SelectedShipper.SelectedItem).Name, ConsigmentNumber.Text).Result;
                if (result)
                {
                    ...
                    await viewModel.GetPalletsByStatus("booked");
                    await DisplayAlert("Message", "Pallet has been dispatched successfully", "Ok");
                }
                else
                {
                    await DisplayAlert("Error", "Dispatch failed", "Ok");
                }
            }
            else
            {
